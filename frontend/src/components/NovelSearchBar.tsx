  ...
  useEffect(() => {
    ...
    const searchRequest = setTimeout(async () => {
      ...
      try {
        ...
        console.error("Error Finding the Novel :", error);
      } finally {
        setIsLoading(false);
      }
    }, 600);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <h3 className="text-lg font-bold text-white">Search Novels</h3>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {isLoading ? (
            <div className="text-center py-10 flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-slate-550 text-sm font-medium">Loading results...</p>
            </div>
          ) 
          : results.length > 0 ? (
            <div className="flex flex-col gap-1 max-h-96 overflow-y-auto scrollbar-hide">
              {results.map((novel) => (
                <div key={novel.id} className="flex items-center gap-4 p-2.5 hover:bg-slate-950/60 rounded-xl transition-colors cursor-pointer border-b border-slate-950/40 last:border-0">
                  <img src={novel.image} alt={novel.title} referrerPolicy="no-referrer" className="w-12 h-16 object-cover rounded-lg border border-slate-800" />
                  <div className="min-w-0 flex-1">
                    <h4 className="text-sm font-bold text-slate-200 truncate">{novel.title}</h4>
                  </div> 
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
